...
import {websiteWidth} from './Container'
...
export default function Header({width = '4/5'}: Props) {
  ...
  return (
    <header className="fixed w-full pt-5 sm:pt-2 text-[22px] sm:text-lg font-book z-50">
      <nav className={`flex items-center justify-between bg-item mx-auto p-4 sm:p-1.5 rounded-[20px] sm:rounded-xl ${websiteWidth[width]}`}>
        <div className="flex items-center gap-8 sm:gap-3">
          <a href="/" className="w-[70px] h-10 sm:w-14 sm:h-8 duration-300 rounded-[12px] sm:rounded-lg bg-secondary hover:bg-primary-hover"></a>
